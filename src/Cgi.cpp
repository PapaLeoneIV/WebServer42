...
void Cgi::setArgs(const std::string &target, const Client *c)
{
...
char **Cgi::generateEnv(const Client *c, const std::string &target)
{
  ...
  if (req->getMethod() == "POST")
  {
    ...
    const std::string cl = headers.count("content-length") ? headers["content-length"] : "";
    const std::string ct = headers.count("content-type") ? headers["content-type"] : "";
...
void Cgi::execute(const Client *c)
{
  ...
  if (this->cgi_pid == 0)
  {
    ...
    close(pipe_out[0]);
    close(pipe_out[1]);
    const int exit_status = execve(this->args[0], this->args, this->envp);
    exit(exit_status);
  }
  if (this->cgi_pid > 0)
  {
...
char **Cgi::getArgs() const { return this->args; }

void Cgi::setEnv(char **envp) { this->envp = envp; }

char **Cgi::getEnv() const { return this->envp; }

void Cgi::setCGIState(const int state) { this->cgi_state = state; }

int Cgi::getCGIState() const { return this->cgi_state; }

int *Cgi::getPipeIn() { return this->pipe_in; }

int *Cgi::getPipeOut() { return this->pipe_out; }

int Cgi::getCgiPid() const { return this->cgi_pid; }

size_t Cgi::getBytesSended() const { return this->bytes_sended; }

void Cgi::incrementBytesSended(const size_t bytes) { this->bytes_sended += bytes; }
